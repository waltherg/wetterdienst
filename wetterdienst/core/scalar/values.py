...
class ScalarValuesCore:
    ...
    def _meta_fields(self) -> List[str]:
        """
        Metadata fields that are independent of actual values and should be parsed differently
        ...
        if not self.stations.stations.tidy:
            ...
            fields = [
                ...
                Columns.QUALITY_SECONDARY.value
            ]
    ...
    def _organize_df_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        columns.extend(df.columns.difference(columns))
    ...
    def query(self) -> Generator[ValuesResult, None, None]:
        ...
        for station_id in self.stations.station_id:
            ...
            for parameter in self.stations.parameter:
                # TODO: remove doubling of parameter, here parameter is (parameter, dataset),
                ...
                if self.stations.stations.tidy:
                    ...
                    if parameter_ != dataset:
                        parameter_df = parameter_df[
                            parameter_df[Columns.PARAMETER.value] == parameter_.value.lower()
                            ]

                parameter_df = self._organize_df_columns(parameter_df)

                parameter_df[Columns.QUALITY_PRIMARY.value] = parameter_df[Columns.QUALITY_PRIMARY.value].astype(float)
                parameter_df[Columns.VALUE.value] = pd.to_numeric(parameter_df[Columns.VALUE.value])
    ...
    def _coerce_meta_fields(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        columns = [Columns.STATION_ID.value]

        if self.stations.stations.tidy:
            columns.extend([Columns.DATASET.value, Columns.PARAMETER.value, Columns.QUALITY_PRIMARY.value])

        for column in columns:
            df[column] = self._coerce_strings(
                df[column]).astype("category")

        if self.stations.stations.tidy:
            df[Columns.VALUE.value] = df[Columns.VALUE.value].astype(float)
